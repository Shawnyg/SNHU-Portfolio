...
// #include <string>       // std::string, std::to_string. Only used it for debugging
...
template <typename T>
T add_numbers(T const& start, T const& increment, unsigned long int const& steps, int &status)
{
    T result = start;
    const T maxNum = std::numeric_limits<T>::max(); // define the max value of the provided datatype
    const T minNum = std::numeric_limits<T>::min(); // define the minimum value of the provided datatype
    status = 0; // 0 = OK, 1 = OVERFLOW, -1 = UNDERFLOW. Set it to OK status before each operation starts

    for (unsigned long int i = 0; i < steps; ++i)
    {
        if (increment >= 0 && result > (maxNum - increment)) {
            // Overflow detected
            status = 1;
        }
...
template <typename T>
T subtract_numbers(T const& start, T const& decrement, unsigned long int const& steps, int &status)
{
    T result = start;
    const T maxNum = std::numeric_limits<T>::max();
    const T minNum = std::numeric_limits<T>::min();
    status = 0; // 0 = OK, 1 = OVERFLOW, -1 = UNDERFLOW. Set it to OK status before each operation starts

    for (unsigned long int i = 0; i < steps; ++i)
    {

        if (result < (minNum + decrement)) {
            // Underflow detected
            status = -1;
        }
        result -= decrement;
...
// Function to output the status of the operation, whether or not an underflow/overflow was detected. The operation will still output the calculated value.
void print_status(int status) { // and yes, inline opening curly bracket is superior to new line curly bracket
    switch (status) {
        case 0:
...
void test_overflow()
{
    ...
    // END DO NOT CHANGE
    int status = 0; // 0 = OK, 1 = OVERFLOW, -1 = UNDERFLOW

    std::cout << "\tAdding Numbers Without Overflow (" << +start << ", " << +increment << ", " << steps << ") = ";
    T result = add_numbers<T>(start, increment, steps, status);
    std::cout << +result << std::endl;

    print_status(status);

    std::cout << "\tAdding Numbers With Overflow (" << +start << ", " << +increment << ", " << (steps + 1) << ") = ";
    result = add_numbers<T>(start, increment, steps + 1, status);
    std::cout << +result << std::endl;
    print_status(status);
...
void test_underflow()
{
    ...
    int status = 0; // 0 = OK, 1 = OVERFLOW, -1 = UNDERFLOW. Status variable to represent the operation's result

    std::cout << "\tSubtracting Numbers Without Overflow (" << +start << ", " << +decrement << ", " << steps << ") = ";
    auto result = subtract_numbers<T>(start, decrement, steps, status);
    std::cout << +result << std::endl;
    print_status(status); // Output the status of the operation

    std::cout << "\tSubtracting Numbers With Overflow (" << +start << ", " << +decrement << ", " << (steps + 1) << ") = ";
    result = subtract_numbers<T>(start, decrement, steps + 1, status);
    std::cout << +result << std::endl;
    print_status(status); // Output the status of the operation
}
