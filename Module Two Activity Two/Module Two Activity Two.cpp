...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
    ...
    // Check for SQL injection for the 'OR' case provided
    size_t found1 = sql.find("or"); // If "or" / "OR" is found in the sql "string"
    size_t found2 = sql.find("OR"); // Figured this way was more as efficient as lowering a string since they both practically visit each character in the string, so might as well have 2 variables

    if (found1 != std::string::npos || found2 != std::string::npos) { // If 'or' or 'OR' were found in the sql "string", terminate the query
        std::cout << "SQL Injection OR encountered, terminating query. Please validate SQL." << std::endl;
